import localizationKeys from "./localization-keys";

//   OurApp: "OurApp",

const en = {
  // auth
  [localizationKeys.LOGIN]: "LOG IN",
  [localizationKeys.Hithere]: "Hi there !",
  [localizationKeys.Logintomanageyourtranslatorslistsandfilterthe]:
    "Log in to manage your translators' lists and filter the",
  [localizationKeys.resultstosuityourdesiredjobtype]:
    " results to suit your desired job type",
  [localizationKeys.FORGOTPASSWORD]: "FORGOT PASSWORD",
  [localizationKeys.RememberMe]: "Remember Me",
  [localizationKeys.OR]: "OR",
  [localizationKeys.CREATENEWACCOUNT]: "CREATE NEW ACCOUNT",
  [localizationKeys.sentverification]: "sent verification",
  [localizationKeys.Email]: "E-mail!",
  [localizationKeys.Password]: "PASSWORD",
  [localizationKeys.NEWAccount]: "NEW Account",
  [localizationKeys.FULLNAME]: "FULL NAME",
  [localizationKeys.PHONE]: "PHONE",
  [localizationKeys.IAGREETOTHETERMSANDCONDITIONS]:
    "I AGREE T THE TERMS AND CONDITIONS",
  [localizationKeys.SIGNUP]: "SIGN UP",
  [localizationKeys.GOTOLOGIN]: "GO TO LOGIN",
  [localizationKeys.RESETPASSWORD]: "RESET PASSWORD",
  [localizationKeys.NewPassword]: "New Password",
  [localizationKeys.ReEnterPassword]: "Re-Enter Password",
  // home
  [localizationKeys.SELL]: "SELL",
  [localizationKeys.Welcome]: "Welcome",
  [localizationKeys.English]: "English",
  [localizationKeys.Arabic]: "Arabic",
  [localizationKeys.AllCategories]: "All Categories",
  [localizationKeys.TRY]: "TRY",
  [localizationKeys.Buysellandfindjustaboutanythingusingtheapponyourmobile]:
    "Buy, sell and find just about any thing using the appon your mobile.",
  [localizationKeys.ChooseType]: "Choose Type",
  [localizationKeys.Choosemake]: "Choose make",
  [localizationKeys.ChooseModel]: "Choose model",
  [localizationKeys.Chooseyear]: "Choose year",
  [localizationKeys.SEARCHNOW]: "SEARCH NOW",
  [localizationKeys.Type]: "Type",
  [localizationKeys.make]: "Make",
  [localizationKeys.Model]: "Model",
  [localizationKeys.year]: "Year",
  [localizationKeys.Keyword]: "Keyword...",
  [localizationKeys.NewUsed]: "New & Used",
  [localizationKeys.New]: "New",
  [localizationKeys.Used]: "Used",
  [localizationKeys.Location]: "Location",
  [localizationKeys.Findamazingdealsonthego]: "Find amazing deals on the go.",
  [localizationKeys.Downloadtheappnow]: "Download the app now!",
  [localizationKeys.Freshrecommendations]: "Fresh recommendations",
  [localizationKeys.Anewexperienceintheworldofsecondhandpurchase]:
    "A new experience in the world of second-hand purchase",
  [localizationKeys.TAKEALOOK]: "TAKE A LOOK!",
  [localizationKeys.Bikesforsale]: "Bikes for sale",
  [localizationKeys.NewsReviews]: "News & Reviews",
  [localizationKeys.GeneralHelp]: "General Help",
  [localizationKeys.OurApp]: "Our App",
  // profile
  [localizationKeys.TotalPageView]: "Total Page View",
  [localizationKeys.logout]: "LOG OUT",
  [localizationKeys.MyAccout]: "My Accout",
  [localizationKeys.REJECTED]: "REJECTED",
  [localizationKeys.PENDING]: "PENDING",
  [localizationKeys.PUBLISHED]: "PUBLISHED",
  [localizationKeys.DRAFTED]: "DRAFTED",
  [localizationKeys.EXPIRED]: "EXPIRED",
  // add ads
  [localizationKeys.Nowchoosetherightcategory]:
    "Now choose the right category ",
  [localizationKeys.foryourad]: "for your ad:",
  [localizationKeys.ConsultantPsychiatristExperienceInAdultPsychiatry]:
    "Consultant Psychiatrist - Experience In Adult Psychiatry",
  [localizationKeys.AndAddictionMedicineITreatMany]:
    "And Addiction Medicine I Treat Many",
  [localizationKeys.next]: "NEXT",
  [localizationKeys.Category]: "Category",
  [localizationKeys.SubCategory]: "Sub Category",
  [localizationKeys.Yourealmostthere]: "You’re almost there!",
  [localizationKeys.Includeasmuchdetailsandpicturesaspossible]:
    "Include as much details and pictures as possible,",
  [localizationKeys.andsettherightprice]: "and set the right price!",
  [localizationKeys.UPLOAD]: "UPLOAD",
  [localizationKeys.YOURPHOTO]: "YOUR PHOTO",
  [localizationKeys.POST]: "POST",
  [localizationKeys.title]: "TITLE",
  [localizationKeys.price]: "PRICE",
  [localizationKeys.DESCRIPTIONYOURAD]: "DESCRIPTION YOUR AD...",
  [localizationKeys.description]: "DESCRIPTION",
  [localizationKeys.brand]: "BRAND",
  [localizationKeys.AddModel]: "MODEL",
  [localizationKeys.AddBrand]: "ADD BRAND",
  [localizationKeys.model]: "ADD MODEL",
  [localizationKeys.city]: "CITY",
  [localizationKeys.usage]: "USAGE",
  [localizationKeys.ITEMOVERVIEW]: "ITEM OVERVIEW",
  [localizationKeys.YOUMAYALSOLIKE]: "YOU MAYAL SO LIKE",
  //
  [localizationKeys.MOREOPTIONYOULIKE]: "MORE OPTION YOU LIKE",
  [localizationKeys.BACKTOCATEGORY]: "BACK TO CATEGORY",
  [localizationKeys.ABOUTTHISDEALER]: "ABOUT THIS DEALER",
  [localizationKeys.CALLNOW]: "CALL NOW",
  [localizationKeys.Emailcopysuccess]: "E-mail copy success",
  [localizationKeys.ASKTHEDEALERAQUESTION]: "ASK THE DEALER A QUESTION",
  [localizationKeys.IMINTERESTEDPLEASECONTACTME]:
    "I'M INTERESTED, PLEASE CONTACT ME?",
  [localizationKeys.CONTACTMEABOUTINSURANCE]: "CONTACT ME ABOUT INSURANCE",
  [localizationKeys.CONFIRMDEALERRESPONSE]: " CONFIRM DEALER'S RESPONSE",
  [localizationKeys.FILLINTHISFORMFORMENEXTTIME]:
    " FILL IN THIS FORM FOR ME NEXT TIME",
  [localizationKeys.SENDENQUIRY]: "SEND ENQUIRY",
  //
  [localizationKeys.Bikesforsale]: "Bikes for sale",
  [localizationKeys.NewsReviews]: "News & Reviews",
  [localizationKeys.GeneralHelp]: "General Help",
  [localizationKeys.OurApp]: "Our App",

  [localizationKeys.Usedbikesforsale]: "Used bikes for sale",
  [localizationKeys.Newbikesforsale]: "New bikes for sale",
  [localizationKeys.BikeFactsHistoryReports]: "BikeFacts History Reports",
  [localizationKeys.BikeNews]: "Bike News",
  [localizationKeys.BikeReviews]: "Bike Reviews",
  [localizationKeys.BikeAdvice]: "Bike Advice",
  [localizationKeys.BikeVideos]: "Bike Videos",
  [localizationKeys.TermsConditions]: "Terms & Conditions",
  [localizationKeys.Privacy]: "Privacy",
  [localizationKeys.ContactUs]: "Contact Us",
  [localizationKeys.AboutUs]: "About Us",
  [localizationKeys.DownloadOurAppNow]: "Download Our App Now",
  [localizationKeys.OnAppStoreGooglePlay]: "On App Store / Google Play",
  //

  [localizationKeys.TALKTOUS]: "TALK TO US",
  [localizationKeys.CONTACT]: "CONTACT",
  [localizationKeys.US]: "US",
  [localizationKeys.TAKEACOFFEEWITHUS]: "TAKE A COFFEE WITH US",
  [localizationKeys.CONTACTUS]: "CONTACT US",
  [localizationKeys.DOYOU]: "DO YOU",
  [localizationKeys.HAVEANY]: "HAVE ANY",
  [localizationKeys.QUESTIONS]: "QUESTIONS",
  [localizationKeys.firstName]: "first Name",
  [localizationKeys.familyName]: "family Name",
  [localizationKeys.subject]: "subject",
  [localizationKeys.MESSAGE]: "MESSAGE",
  [localizationKeys.SEND]: "SEND",
  [localizationKeys.ForBuyers]: "For Buyers",
  [localizationKeys.ByBuyers]: "By Buyers",
  [localizationKeys.itisalongestablishedfactthatareaderwillbe]:
    "it is a long established fact that a reader will be",
  [localizationKeys.distractedbythereadablecontent]:
    "distracted by the readable content",
  [localizationKeys.ofapagewhenlookingatitslayoutthepointofusing]:
    "of a page when looking at its layout. the point of using",
  [localizationKeys.loremipsumisthatithasa]: "lorem ipsum is that it has a",
  //
  [localizationKeys.OURSTORY]: "OUR STORY",
  [localizationKeys.ABOUTUS]: "ABOUT US",

  [localizationKeys.buy]: "buy",
  [localizationKeys.LEARNMORE]: "LEARN MORE",
  [localizationKeys.SUBMIT]: "SUBMIT",
  //
  [localizationKeys.ALLMANUFACTURES]: "ALL MANUFACTURES",
  [localizationKeys.SearchRuslt]: "Search Ruslt",
  [localizationKeys.TRANSMISSIONTYPEF]: "TRANSMISSION TYPEF",
  [localizationKeys.PRICE]: "PRICE",
};

export default en;
